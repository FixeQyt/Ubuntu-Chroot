...
class CommandExecutor {
  constructor() {
    const scriptTag = document.querySelector(
      'script[src*="command-executor.js"]',
    );
    this.assetsPath = scriptTag
      ? scriptTag.src.replace(/\/command-executor\.js$/, "")
      : "./assets";
    ...
    if (this.execMethod === "ksu") {
      ...
      window[callback] = (exitCode, stdout, stderr) => {
        ...
        if (exitCode === 0) {
          ...
          if (onComplete)
            onComplete({ success: true, exitCode, output: stdout });
        } else {
          // --- FIXED ---
          ...
          if (onComplete)
            onComplete({ success: false, exitCode, error: errorMessage });
        }
      };
      ...
      try {
        ...
        window.SULib.exec(fullCommand, (result) => {
          ...
          if (result.success) {
            ...
            if (onComplete)
              onComplete({ success: true, output: result.output });
          } else {
            // --- FIXED ---
